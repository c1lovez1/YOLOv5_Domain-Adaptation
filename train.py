...
def train(hyp, opt, device, callbacks):
    ...
    optical_path = None
    sar_path = None
    if isinstance(data_dict.get('train'), dict) and 'optical' in data_dict['train'] and 'sar' in data_dict['train']:
        optical_path = data_dict['train']['optical']
        sar_path = data_dict['train']['sar']
        train_path = None  # 设置为None，因为我们已经有了optical_path和sar_path
        LOGGER.info(f"Using optical path: {optical_path} and SAR path: {sar_path}")
    else:
        train_path = data_dict['train']
        LOGGER.info(f"Using train path: {train_path}")
    ...
    train_loader, dataset = create_dataloader(
        ...
        shuffle=True,
        seed=opt.seed,
        optical_path=optical_path,  # 添加光学图像路径
        sar_path=sar_path,          # 添加SAR图像路径
    )
...
def parse_opt(known=False):
    ...
    parser.add_argument("--optical-path", type=str, help="源域（光学图像）的路径")
    parser.add_argument("--sar-path", type=str, help="目标域（SAR图像）的路径")
