...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        # 如果提供了光学和SAR路径，则使用它们进行PITM
        if optical_path and sar_path:
            dataset = LoadImagesAndLabels(
                optical_path=optical_path,  # 光学图像路径
                sar_path=sar_path,          # SAR图像路径
                img_size=imgsz,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.stride = stride
        self.path = path if path is not None else optical_path  # 如果path为None，则使用optical_path
        self.albumentations = Albumentations(size=img_size) if augment else None

        try:
            if optical_path is not None and sar_path is not None:
                # 读取光学图像和SAR图像
                self.optical_paths = glob.glob(os.path.join(optical_path, '*'))
                self.sar_paths = glob.glob(os.path.join(sar_path, '*'))
                self.im_files = self.optical_paths + self.sar_paths
                self.im_files = sorted(x.replace("/", os.sep) for x in self.im_files if x.split(".")[-1].lower() in IMG_FORMATS)
                ...
                raise ValueError(f"{prefix}Either path or both optical_path and sar_path must be provided")
            
            assert self.im_files, f"{prefix}No images found"
        except Exception as e:
            path_info = optical_path if path is None else path
            raise Exception(f"{prefix}Error loading data from {path_info}: {e}\n{HELP_URL}") from e
        ...
        if len(self.label_files) > 0:
            ...
            # 如果没有标签文件，则使用图像路径的父目录
            cache_path = Path(self.im_files[0]).parent.with_suffix(".cache") if len(self.im_files) > 0 else Path(self.path).with_suffix(".cache")
        
        try:
            cache, exists = np.load(cache_path, allow_pickle=True), True  # load dict
            if isinstance(cache, np.ndarray):
                cache = {'data': cache}  # 兼容旧版本
            if isinstance(cache, dict) and 'data' in cache:
                cache = cache['data'].item()  # 获取数据项
            elif isinstance(cache, np.lib.npyio.NpzFile):
                cache = dict(cache)  # 转换为字典
                
            assert cache.get("version", 0) == self.cache_version  # matches current version
            assert cache.get("hash") == get_hash(self.label_files + self.im_files)  # identical hash
        except Exception as e:
            LOGGER.warning(f"Warning: Cache loading error: {e}")
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        with Pool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segs, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                if im_file:
                    x[im_file] = [lb, shape, segs]
                if msg:
                    msgs.append(msg)
    ...
    def enhance_optical_image(self, img):
        """使用CLAHE增强光学图像"""
    ...
    def extract_sar_background(self, img):
        """提取SAR图像的背景"""
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        return binary

    def synthesize_intermediate_domain(self, optical_img, sar_img):
        """合成中间域图像"""
        sar_background = self.extract_sar_background(sar_img)
        # 随机选取一个SAR背景图像并与光学图像融合
        combined = cv2.addWeighted(optical_img, 0.5, sar_background, 0.5, 0)
        return combined
